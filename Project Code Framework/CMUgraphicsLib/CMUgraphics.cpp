...
static LRESULT CALLBACK WindowProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
	...
	switch(msg) {
		//WM_SYSCOMMAND added by karim to detect the X button on the title bar of the window
	  case WM_SYSCOMMAND:
		  if(wParam==SC_CLOSE)
		  {
			  if(wipInput != NULL) {
				  wipInput->SetUserClickedClose(true);
	  ...
	  case WM_LBUTTONDOWN:
        if(wipInput != NULL) {
            wipInput->SetMouseState(hwnd, LEFT_BUTTON, BUTTON_DOWN, GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam));
		}
		SetCapture(hwnd);
 		return 0;
	  ...
	  case WM_RBUTTONDOWN:
        if(wipInput != NULL) {
            wipInput->SetMouseState(hwnd, RIGHT_BUTTON, BUTTON_DOWN, GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam));
		}
		SetCapture(hwnd);
 		return 0;	
	  ...
	  case WM_LBUTTONUP:
        if(wipInput != NULL) {
            wipInput->SetMouseState(hwnd, LEFT_BUTTON, BUTTON_UP, GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam));
            wipInput->SetClickInfo(hwnd, LEFT_CLICK, GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam));
		}
		ReleaseCapture();
 		return 0;
	  ...
	  case WM_RBUTTONUP:
        if(wipInput != NULL) {
            wipInput->SetMouseState(hwnd, RIGHT_BUTTON, BUTTON_UP, GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam));
            wipInput->SetClickInfo(hwnd, RIGHT_CLICK, GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam));
		}
		ReleaseCapture();
 		return 0;
	  ...
	  case WM_MOUSEMOVE:
        if(wipInput != NULL) {
            wipInput->SetMouseCoord(hwnd,  GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam));
		}
 		return 0;
	  ...
	  case WM_KEYDOWN:
	    switch(wParam) {
		  ...
		  case VK_DELETE:
				if(wipInput != NULL) {
					wipInput->SetKeyInfo(hwnd, ASCII, 127);
				}
		   break;
		  case VK_F1:
            if(wipInput != NULL) {
...
inline void window::ProcessMessage() {
    ...
    MSG msgStruct;
	//Edit By karim : making the message peek Take null instead of hwndWindow ,so messages of the whole thread are proccessed
	//Fixes the stupid windows not reponsding in case of multiple windows
    if(PeekMessage(&msgStruct, NULL, 0, 0, PM_REMOVE)) {
	//if(GetMessage(&msgStruct,NULL,0,0)){
	    TranslateMessage(&msgStruct);
		DispatchMessage(&msgStruct);
	}
}

inline void window::ProcessMessage2() {

    MSG msgStruct;
	//Edit By karim : use get instead of peek msg
...
clicktype window::WaitMouseClick(int &iX, int &iY) {
	...
	while(true) {
		if(wipInput->UserClickedClose)
		{
			wipInput->UserClickedClose=false;
			return CLOSE_CLICK;
		}
	    mqueTmp = mqueInput.Remove();
	    if(mqueTmp != NULL) {
...
int window::Display_MessageBox(string msg,string title,UINT Type)
{
	return MessageBox(this->hwndWindow,(char*)msg.c_str(),(char*)title.c_str(),Type);
}

bool window::GetOpenFilename(string& str)
{
	...
	ofn.lpstrFilter = "Logic Design File (.logic)\0*.logic\0";
	ofn.nFilterIndex = 1;
	ofn.lpstrFileTitle = NULL;
	ofn.nMaxFileTitle = 0;
	ofn.lpstrInitialDir = NULL;
	ofn.Flags = OFN_PATHMUSTEXIST | OFN_FILEMUSTEXIST | OFN_NOCHANGEDIR; //Warning : OFN_NOCHANGEDIR is not effective for Windows XP
...
bool window::GetSaveFilename(string& str)
{
	OPENFILENAME ofn;       // common dialog box structure
	char szFile[260];       // buffer for file name
	HWND hwnd;              // owner window
...
bool window::GetExportFilename(string& str)
{
	OPENFILENAME ofn;       // common dialog box structure
	char szFile[260];       // buffer for file name
	HWND hwnd;              // owner window
